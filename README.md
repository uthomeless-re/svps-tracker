...
- **svlabo.jp**: 順位フィルタを「100位以上」に固定しています。ランクマッチで100位以内に
  ...
  また、初回実行時にFC2側のボット検知で「このページの表示は許可されていません」という
  拒否ページに飛ばされる事象を確認しました。User-Agent偽装・navigator.webdriver隠蔽・
  トップページ経由アクセスなどの対策を入れていますが、**GitHub Actionsのデータセンター系IP自体が
  ブロックされている場合はコード側の対策では解決できません**。何度実行しても
  `[svlabo] still blocked after retry` が出る場合は、この取得元は諦めて手動更新に切り替えるか、
  ご自身のPC等（データセンターIPでない環境）でスクリプトを実行する運用に変更してください。
